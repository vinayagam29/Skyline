...
const Nav = () => {
  ...
  return (
    <>
      {/* Navbar */}
      <div
        className={`flex items-center justify-between h-19.5 lg:px-24 md:px-4 px-3 roboto-condensed-medium text-[14px] sticky top-0 z-50 bg-white/90 shadow-md transition-transform duration-300 ${
          showNavbar ? "translate-y-0" : "-translate-y-full"
        }`}
      >
        
        <Link to={"/"}>
          <img className="md:scale-100 scale-80" src="/imgsky/Layer_1.svg" alt="Logo" />
        </Link>
      ...
      <div
        className={`fixed z-40 top-0 left-0 w-full h-screen bg-white/90 flex flex-col items-center justify-center gap-10 text-lg roboto-condensed-medium transition-transform duration-500 ${
      ...
      {openModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative animate-fadeIn">
            ...
            {!submitted ? (
              <>
                <h2 className="text-2xl roboto-condensed-semibold mb-6 text-center">Enquire Now</h2>
              ...
              <div className="text-center py-10">
                <div className="text-5xl mb-4 text-[#F38831]">✓</div>
                <h2 className="text-2xl roboto-condensed-semibold mb-2">
                  Thank You, <span className="text-[#F38831]">{formData.name}</span>!
                </h2>
